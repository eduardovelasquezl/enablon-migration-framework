"""Adaptadores de Safety Meetings para el Framework Core v1 (Sprint 9.7).

Solo dos etapas (`query`, `transform_and_export`) -- mismo alcance que
Bypass: ni `canonicalize` ni `evidence` (no implementada, ver
docs/07-developer-guide/safety-meetings-module.md § 7). Vive en la capa de
Plugin (`src/export/prototype/safety_meetings/`), nunca en `src/core/`.

La etapa `query` usa `GenericQueryStage` (Export Engine,
`src/export/engine/query_stage.py`) desde el primer commit de este módulo
-- no hay migración que hacer, nace ya sobre el Engine (a diferencia de
Drills/Bypass, que lo adoptaron en Sprint 9.6). `transform_and_export`
sigue envolviendo `pipeline.run()` completo -- MODULE_SPECIFIC, mismo
patrón que Bypass (deuda técnica conocida y deliberadamente no extraída,
ver Sprint 9.5.1/9.6/9.7)."""
...
from .config import load_safety_meetings_config
from .extractor import ExtractionResult, extract_safety_meetings
from .pipeline import PipelineResult as SafetyMeetingsRunResult
from .pipeline import run as run_safety_meetings_pipeline
...
OBJECT_TYPE = "safety_meetings"
SOURCE_SYSTEM = "prevencion"

DEFAULT_OUTPUT_ROOT = PROJECT_ROOT / "outputs" / "prototype" / "safety_meetings"
...
def _build_safety_meetings_query_stage() -> GenericQueryStage:
    return GenericQueryStage(QueryStageSpec(
        name=STAGE_QUERY,
        config_loader=load_safety_meetings_config,
        state_key="safety_meetings_config",
        filter_catalog=SAFETY_MEETINGS_FILTER_CATALOG,
        extract_fn=extract_safety_meetings,
    ))


class SafetyMeetingsTransformExportStage:
    """Envuelve `pipeline.run()` completo -- mismo rol que
    `BypassTransformExportStage`, ADAPTADOR TEMPORAL sobre el mismo tipo de
    deuda ya documentada para Drills/Bypass (framework-core-v1.md § 15)."""
    ...
    def execute(self, context: ExecutionContext, stage_input: ExtractionResult) -> StageResult:
        ...
        try:
            result = run_safety_meetings_pipeline(
        ...
        artifacts = [
            ArtifactReference(name="safety_meetings.csv", path=result.csv_path, kind="csv", stage=self.name),
...
def register_safety_meetings_stages(registry: StageRegistry, *, overwrite: bool = False) -> None:
    registry.register(STAGE_QUERY, _build_safety_meetings_query_stage(), overwrite=overwrite)
    registry.register(STAGE_TRANSFORM_AND_EXPORT, SafetyMeetingsTransformExportStage(), overwrite=overwrite)


def build_safety_meetings_pipeline_definition() -> PipelineDefinition:
    ...
    return PipelineDefinition(name="safety_meetings", stages=stages)
...
def build_execution_context(
    ...
    return ExecutionContext(
        ...
        logger=logger or logging.getLogger("src.export.prototype.safety_meetings.core_adapters"),
    )
