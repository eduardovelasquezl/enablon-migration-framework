SELECT 
    S.IDSolicitudCambio,
    CONCAT(S.IDSolicitudCambio, '.S') AS IDEncuestaS,
    CONCAT(S.IDSolicitudCambio, '.S.', X.IDMaestro) AS IDPreguntaEncuesta,
...
CROSS APPLY (
    ...
    FROM (
        SELECT CAST('<X>' + REPLACE(S.EncuestasS, ',', '</X><X>') + '</X>' AS XML) AS Data
    ) A
